...
<ul class="nav nav-tabs" role="tablist" id="maintabs">
    <li <?php if ((empty($showIntro) ? ('0') : ($showIntro)) == '1') { ?>class="active"<?php } ?>><a data-toggle="tab" id="certificates-introduction" href="#subtab_certificates-introduction"><b><?= $lang->_('Introduction') ?></b></a></li>
    <li <?php if ((empty($showIntro) ? ('0') : ($showIntro)) == '0') { ?>class="active"<?php } ?>><a data-toggle="tab" id="certificates-tab" href="#certificates"><b><?= $lang->_('Certificates') ?></b></a></li>
</ul>

<div class="content-box tab-content">

    <div id="subtab_certificates-introduction" class="tab-pane fade <?php if ((empty($showIntro) ? ('0') : ($showIntro)) == '1') { ?>in active<?php } ?>">
        <div class="col-md-12">
            ...
            <ul>
              <li><?= sprintf($lang->_('Certificates must be %svalidated%s by the CA before they can be used. This process runs in the background and may take several minutes to complete. The progress can be monitored by using the %slog files%s.'), '<b>', '</b>', '<a href="/ui/acmeclient/logs">', '</a>') ?></li>
              <li><?= sprintf($lang->_('Certificates are stored in the %sOPNsense certificate storage%s. When a CA has completed the validation of a certificate request, the resulting certificate is then automatically imported into the OPNsense certificate storage. The same applies when renewing certificates, the existing entry in the OPNsense certificate storage will automatically be updated.'), '<a href="/system_certmanager.php">', '</a>') ?></li>
              <li><?= sprintf($lang->_('When removing a certificate from the plugin, the certificate in the %sOPNsense certificate storage%s is %sNOT removed%s, because it may still be used by a core application or another plugin. Obsolete certificates should be manually removed from the OPNsense certificate storage. Note that when creating a new certificate with the same name, a new certificated will be imported into the OPNsense certificate storage (instead of updating the existing entry).'), '<a href="/system_certmanager.php">', '</a>', '<b>', '</b>') ?></li>
            </ul>
            <p><?= sprintf($lang->_('When experiencing issues, try setting the log level to "debug" on the %ssettings%s page.'), '<a href="/ui/acmeclient#settings">', '</a>') ?></p>
        </div>
    </div>

    <div id="certificates" class="tab-pane fade <?php if ((empty($showIntro) ? ('0') : ($showIntro)) == '0') { ?>in active<?php } ?>">
...
<?= $this->partial('layout_partials/base_dialog', ['fields' => $formDialogCertificate, 'id' => 'DialogCertificate', 'label' => $lang->_('Edit Certificate')]) ?>
