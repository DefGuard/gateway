...
<ul class="nav nav-tabs" role="tablist" id="maintabs">
    <li <?php if ((empty($showIntro) ? ('0') : ($showIntro)) == '1') { ?>class="active"<?php } ?>><a data-toggle="tab" id="validations-introduction" href="#subtab_validations-introduction"><b><?= $lang->_('Introduction') ?></b></a></li>
    <li <?php if ((empty($showIntro) ? ('0') : ($showIntro)) == '0') { ?>class="active"<?php } ?>><a data-toggle="tab" id="validations-tab" href="#validations"><b><?= $lang->_('Challenge Types') ?></b></a></li>
</ul>

<div class="content-box tab-content">

    <div id="subtab_validations-introduction" class="tab-pane fade <?php if ((empty($showIntro) ? ('0') : ($showIntro)) == '1') { ?>in active<?php } ?>">
        <div class="col-md-12">
            ...
            <ul>
              <li><?= sprintf($lang->_('%sDNS-01:%s This is the most reliable challenge type and thus highly recommended when using this plugin. It requires that you control the DNS for your domain name and that your DNS provider is supported both %sby acme.sh%s and this plugin.'), '<b>', '</b>', '<a href="https://github.com/acmesh-official/acme.sh/wiki/dnsapi" target="_blank">', '</a>') ?></li>
              <li><?= sprintf($lang->_('%sHTTP-01:%s This challenge type usually requires manual configuration and is not recommended. The DNS name used in the certificate must point to the OPNsense host where the ACME Client plugin is running on. The integrated web service will try to guess the correct settings for your setup, but this may not always work out-of-the-box. Furthermore this challenge type cannot be used to validate %swildcard certificates with Let\'s Encrypt%s.'), '<b>', '</b>', '<a href="https://letsencrypt.org/docs/challenge-types/#http-01-challenge" target="_blank">', '</a>') ?></li>
              <li><?= sprintf($lang->_('%sTLS-ALPN-01:%s This works similar to the HTTP-01 challenge type and has the same requirements. It works if port 80 is unavailable. Other challenge types should be preferred. This challenge type cannot be used to validate %swildcard certificates with Let\'s Encrypt%s.'), '<b>', '</b>', '<a href="https://letsencrypt.org/docs/challenge-types/#tls-alpn-01" target="_blank">', '</a>') ?></li>
            </ul>
            <p><?= sprintf($lang->_('When experiencing issues with a challenge type, try setting the log level to "debug". Please provide full logs when %sreporting issues%s for a challenge type. You should also consider to ask the Certificate Authority for support, if you choose to use a commercial CA.'), '<a href="https://github.com/opnsense/plugins/issues">', '</a>') ?></p>
        </div>
    </div>

    <div id="validations" class="tab-pane fade <?php if ((empty($showIntro) ? ('0') : ($showIntro)) == '0') { ?>in active<?php } ?>">
...
<?= $this->partial('layout_partials/base_dialog', ['fields' => $formDialogValidation, 'id' => 'DialogValidation', 'label' => $lang->_('Edit Challenge Type')]) ?>
