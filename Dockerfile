...
RUN apt-get update && apt-get upgrade -y && \
  apt-get -y install protobuf-compiler libnftnl-dev libmnl-dev pkg-config libssl-dev libudev-dev
WORKDIR /app
COPY . .
...
ARG CACHEBUST=0
RUN apt-get update && apt-get upgrade -y && \
    apt-get -y --no-install-recommends install iproute2 wireguard-tools sudo ca-certificates \
      iptables ebtables nftables lsb-release libudev1 && \
